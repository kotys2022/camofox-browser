 ...
 * (FIXES.md #9). Returns `{}` on empty/invalid input so callers transparently
 ...
 * an array of discrete proxy objects (FIXES.md #10). Deduped by scheme://host:port.
...
function loadConfig({ configPath = CONFIG_PATH } = {}) {
  ...
  return {
    ...
    // `maxBytes` overrides this. See lib/evaluate-projection.js / FIXES.md #2.
    ...
    // is lost on `docker run --rm` without a volume; the engine warns once (FIXES.md).
    ...
    // warm indefinitely (FIXES.md #8) -- disables idle shutdown + enables re-warm.
    ...
    // (FIXES.md #5). The vnc plugin still overrides this with its own resolution.
    ...
    // redaction + length cap, so agent runs are debuggable (FIXES.md #7). Off by
    ...
    proxy: (() => {
      ...
      // so you can paste a full URL yet still override a single field (FIXES.md #9).
      ...
      // Pool of distinct full-proxy endpoints (FIXES.md #10). When present with no
    ...
    // be enabled by its own env var (ENABLE_<PLUGIN>), not just VNC (FIXES.md #1).
