 ...
 *
 *   BROWSER LIFECYCLE
 *     browser:launchOptions   { launchArgs }                   -- mutate launchOptions() input (fingerprint, config)
 ...
 *     tab:captured            { userId, tabId, url, status }         -- capture-response tool
 *
 *   INPUT
 ...
 *
 * Mutating hooks (browser:launchOptions, browser:launching, session:creating)
 * pass their payload object by reference -- plugins can modify it in place
 * before core uses it. browser:launchOptions mutates the launchOptions() input
 * (the seam for persistent fingerprint/config); browser:launching mutates the
 * already-resolved options (env chunks, firefoxUserPrefs, proxy).
 */
